...
def estimate_pose_from_matches(x0: np.ndarray, x1: np.ndarray, K: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
    ...
    # ---- 1) Normalize to camera-bearing coords via K^-1 ----
    Kinv = np.linalg.inv(K)
    x0h = np.hstack([x0, np.ones((N,1))])                  # (N,3)
    x1h = np.hstack([x1, np.ones((N,1))])                  # (N,3)
    x0b = (Kinv @ x0h.T).T                                 # (N,3)
    x1b = (Kinv @ x1h.T).T                                 # (N,3)

    # ---- 2) 8-point algorithm for E (x1^T E x0 = 0) ----
    u0, v0 = x0b[:,0], x0b[:,1]
    u1, v1 = x1b[:,0], x1b[:,1]
    A = np.stack([
        u1*u0, u1*v0, u1,
    ...
    # Enforce rank-2 (last singular value = 0)
    U, S, Vt = np.linalg.svd(E)
    # Optional: make first two singular values equal (closer to true essential)
    s = (S[0] + S[1]) * 0.5
    E = U @ np.diag([s, s, 0.0]) @ Vt

    # ---- 3) Decompose E into (R,t) candidates ----
    ...
    # ---- 4) Linear triangulation & cheirality test; pick best ----
    P0 = np.hstack([np.eye(3), np.zeros((3,1))])           # cam0: [I|0] in normalized coords

    def triangulate_batch(P0, P1, xb0, xb1):
        X = np.zeros((xb0.shape[0], 3))
        for i in range(xb0.shape[0]):
            x, y   = xb0[i,0], xb0[i,1]
            xp, yp = xb1[i,0], xb1[i,1]
            A = np.vstack([
                x * P0[2] - P0[0],
                y * P0[2] - P0[1],
                xp * P1[2] - P1[0],
    ...
    for (R, tvec) in candidates:
        P1 = np.hstack([R, tvec.reshape(3,1)])
        X = triangulate_batch(P0, P1, x0b, x1b)
        z0 = X[:,2]
        X1 = (R @ X.T + tvec.reshape(3,1)).T
        z1 = X1[:,2]
        cheir = (z0 > 0) & (z1 > 0)
        cnt = int(np.sum(cheir))
        if cnt > best_count:
            best_count = cnt
            best = (R, tvec)

    R_best, t_best = best
    # Safety: ensure R is a rotation and t is unit
    if np.linalg.det(R_best) < 0:
        R_best *= -1
        t_best *= -1
    t_best = t_best / (np.linalg.norm(t_best) + 1e-12)

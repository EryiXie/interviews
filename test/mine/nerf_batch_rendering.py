...
def nerf_volume_render(sigmas: np.ndarray, rgbs: np.ndarray, deltas: np.ndarray) -> tuple[float, np.ndarray]:
    ...
    alpha_i = 1.0 - np.exp(-tau)

    cumsum_tau = np.cumsum(tau)     # [τ1, τ1+τ2, τ1+τ2+τ3, ...] # inlcusive sum
    exclusive  = np.concatenate(([0.0], cumsum_tau[:-1])) # move and get exlusive sum
    T_i = np.exp(-exclusive)

    W_i = T_i*alpha_i

    rgb = (W_i[:, None]*rgbs).sum(axis=0)
    alpha  = float(np.clip((W_i).sum(), 0.0, 1.0))
